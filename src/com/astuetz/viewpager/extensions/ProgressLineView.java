...
public class ProgressLineView extends View implements OnPageChangeListener {
	...
	private int mFadeOutTime = 0; // default: do not fade
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		if (mPager != null) {
			...
			final float fullWidth = (mPager.getWidth() + mPager.getPageMargin()) * (mPager.getAdapter()
					.getCount() - 1);

			mLineWidth = w / (fullWidth / currentX);
	...
	public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
		...
		mLineWidth = getMeasuredWidth() / (fullWidth / currentX);
